...
// Buscar usuário e setor
$sql_user = "SELECT nome, setor, inicio_da_jornada, final_da_jornada FROM usuarios_3gre WHERE matricula = ?";
...
if ($linha_registro = $result_check->fetch_assoc()) {
    ...
    // Primeira batida: insere com hora_entrada atual e setor
    $hora_entrada = $hora_agora;
    $sql_insert = "INSERT INTO registros_de_pontos (matricula, data, nome, setor, hora_entrada) VALUES (?, ?, ?, ?, ?)";
    $stmt_insert = $conn->prepare($sql_insert);
    $stmt_insert->bind_param("sssss", $matricula, $data_hoje, $nome, $setor, $hora_entrada);
    $stmt_insert->execute();
}

// Cálculo de progressão do dia
$inicio = strtotime($inicio_jornada);
$fim = strtotime($final_jornada);
